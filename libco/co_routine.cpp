...
/**
 * coctx_swap 接受两个参数，无返回值。其中，第一个参数 curr 为当前协程的 coctx_t
 * 结构指针，其实是个输出参数，函数调用过程中会将当前协程的 context
 * 保存在这个参数指向的内存里；第二个参数 pending，即待切入的协程的 coctx_t
 * 指针，是个输入参数， coctx_swap 从这里取上次保存的
 * context，恢复各寄存器的值。
 */
extern "C" {
...
// 跟单个线程绑定，每个线程上可以跑多个协程，这些协程之间是共享Ent_t的
struct stCoRoutineEnv_t {
  stCoRoutine_t *pCallStack[128];                       // 记录协程之间的返回关系,因为实现的是非对称协程,借用栈实现call-return关系
  int iCallStackSize;
  stCoEpoll_t *pEpoll;
...
// 使用epoll/kqueue实现定时功能
/**
 * 使用 epoll 加时间轮的实现定时器的算法如下：

    Step 1 [epoll_wait] 调用epoll_wait() 等待 I/O 就绪事件，最大等待时长设置为 1
    毫秒（即e poll_wait() 的 第 4 个参数）。 Step 2 [处理 I/O 就绪事件] 循环处理
    epoll_wait() 得到的 I/O 就绪文件描述符。 Step 3 [从时间轮取超时事件]
    从时间轮取超时事件，放到 timeout 队列。 Step 4 [处理超时事件] 如果 Step 3
    取到的超时事件不为空，那么循环处理 timeout 队列中的定时任务。否则跳转到 Step 1
    继续事件循环。 Step 5 [继续循环] 跳转到 Step 1，继续事件循环。
*/
// ----------------------------------------------------------------------------
struct stTimeoutItemLink_t;
struct stTimeoutItem_t;
// 一个全局性的资源，被同一个线程上所有协程共享
struct stCoEpoll_t {
  int iEpollFd;                                     // epoll fs
  static const int _EPOLL_SIZE = 1024 * 10;         // epoll_wait() 系统调用的第三个参数，即一次 epoll_wait 最多返回的就绪事件个数
...
// 创建一个coroutine,该coroutine的标识是ppco,相关属性设置是attr,执行的函数体是pfn,参数是arg
...
// 协程让出只能从当前协程让到上一个resume的地方也就是在栈顶的协程
...
void co_swap(stCoRoutine_t *curr, stCoRoutine_t *pending_co) {
  ...
  // swap context
  coctx_swap(&(curr->ctx), &(pending_co->ctx)); // 开始执行新的协程代码
...
void co_init_curr_thread_env() {
  ...
  env->pCallStack[env->iCallStackSize++] = self;  // 主协程初始化!!
...
// 回调,从主协程切换到就绪协程,还是在一个线程内执行了该回调,不存在线程安全的问题....这个线程即接管了io又做了定时又做了调度器的功能
