...
report_connect(struct socket_server *ss, struct socket *s, struct socket_message *result) {
	...
	/*
		getsockopt 来检测集合中的套接口上是否存在待处理的错误，如果连接建立是成功的，
		则通过 getsockopt(sockfd,SOL_SOCKET,SO_ERROR,(char *)&error,&len) 获取的 error 值将是 0 ，
		如果建立连接时遇到错误，则 error 的值是连接错误所对应的 errno 值，比如ECONNREFUSED，ETIMEDOUT等。  
	*/
...
socket_server_poll(struct socket_server *ss, struct socket_message * result, int * more) {
	for (;;) {
		...
		//每次取出event_n个就绪事件，event_index指向待处理
...
send_request(struct socket_server *ss, struct request_package *request, char type, int len) {
	...
	for (;;) {
		//往管道写命令，写入的字节包括header的两个字节，因此是len+2，最开始是命令字符，len表示包内的结构体长度
...
socket_server_connect(struct socket_server *ss, uintptr_t opaque, const char * addr, int port) {
	...
	if (len < 0)
		return -1;
	//发送'O'，请求包包含相关信息，使用到了柔性数组，因此结构体长度需要加上len
